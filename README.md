...
scheduler.py          # 定时调度入口
  └── collector.py    # 核心采集逻辑
        ├── collect_cnpc()    中石油 API 拦截 + RSA 解密
        ├── collect_sinopec() 中石化 DOM 提取 + 分页
        └── collect_cnooc()   中海油 API 拦截 + DOM 降级
              ↓
        storage.py     # SQLite 去重存储 + Excel 导出
        notifier.py    # QQ 邮箱 SMTP 日报推送
        model_switcher.py  # 模型自动切换
        utils.py       # 关键词匹配等工具函数
```
...
2. 中石油网站会间歇性对直接访问 URL 进行限制，若页面空白需手动导航到其他页面再返回
