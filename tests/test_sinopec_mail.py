...
from config import (
    ...
    DETAIL_URL_TEMPLATES, AI_SUMMARY_ENABLED
)
...
def is_it_related_batch_with_correction(titles):
    """批量判断 IT 相关性，包含关键词复核"""
    from config import IT_KEYWORDS, IT_EXCLUDE_KEYWORDS
...
def collect_sinopec_test(max_pages=5):
    """采集中石化数据（测试版，限制页数）"""
    print("\n" + "=" * 80)
    print("开始采集 中石化 数据（测试版）")
    print("=" * 80)

    base_url = "https://ebidding.sinopec.com"
    tender_url = "https://ebidding.sinopec.com/v3/portal/#/category/NOTICE/TENDER"
    user_data_dir = r"E:\nandaoshuo\oil_tender\api_test\browser_data_sinopec_test"
...
if __name__ == "__main__":
    records = collect_sinopec_test(max_pages=3)

    # 保存结果
    output_file = r"E:\nandaoshuo\oil_tender\api_test\sinopec_test_result.json"
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(records, f, ensure_ascii=False, indent=2)
    print(f"\n结果已保存到：{output_file}")

    # 发送邮件
    send_test_email(records, "中石化")
