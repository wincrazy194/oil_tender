...
def test_cnpc():
    ...
    with sync_playwright() as p:
        ...
        def log_all_responses(response):
            if '/cms/' in response.url or 'article' in response.url:
                api_logs.append(f"{response.status} - {response.url[-50:]}")
            return False  # 不拦截，只是记录
